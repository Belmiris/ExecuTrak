...
Begin VB.Form frmZZSEMPNS 
   ...
   Begin DBTrueGrid.TDBGrid tblComboDropdown 
      Bindings        =   "ZZSEMPNS.frx":1745
      Height          =   2484
      Left            =   0
      OleObjectBlob   =   "ZZSEMPNS.frx":175F
      TabIndex        =   21
      TabStop         =   0   'False
      Top             =   0
      Width           =   3756
...
Private Sub Form_Load()
    ...
    #If FACTOR_MENU = 0 Then
        ...
        t_szConnect = "ODBC;DSN=dktest;DB=/factor/testing/factor;HOST=ether;SERV=sqlexec;SRVR=ether;PRO=sesoctcp;UID=hzheng;PWD=shangyu"
    #End If
...
Private Sub subAfterAddEditClick()
    ...
    If t_nFormMode = ADD_MODE Then
        ...
        tgmDetail.AddEditColumn COL_PERIOD, "Enter a period", szLongPattern
    End If
    ...
    If t_nFormMode = EDIT_MODE Then
        ...
        tgmDetail.RemoveEditColumn COL_PERIOD
    End If
    ...
    tgcDropdown.SQL = "select unique gfsd_series from zzse_gf_store_data where gfsd_flag = " & tfnSQLString(t_nCurrentRpt)
...
Private Function fnValidPeriod(ByVal sText As String, ByVal nRow As Long)
    ...
    i = sText
    If i > 100 Then
        tgmDetail.ErrorMessage(COL_PERIOD) = "The period must be between 0-100"
        Exit Function
    End If
...
Private Function fnPeriodInUse(ByVal sText As String) As Boolean
    ...
    strSQL = "SELECT gfsd_period FROM zzse_gf_store_data " _
            & " WHERE gfsd_flag =" & tfnSQLString(t_nCurrentRpt) _
...
Private Function fnValidNumber(ByVal sText As String, ByVal nRow As Long)
   ...
   '2
    ...
    '3
    If sText > 1000 Then
        tgmDetail.ErrorMessage(COL_NBR) = "The number of stores must be between 0-1000"
       Exit Function
    End If
...
Private Function fnLoadData() As Boolean
    ...
    lytd = 0
    strSQL = "SELECT gfsd_period, gfsd_nbr from zzse_gf_store_data " _
             & " where gfsd_flag = " & tfnSQLString(t_nCurrentRpt) _
             & " and gfsd_series = " & tfnSQLString(txtSeries.Text) _
    ...
    If Not rsTemp Is Nothing Then
        With rsTemp
             If .RecordCount > 0 Then
                ...
                Wend
            Else
               MsgBox " No data in the zzse_gf_store_data table", vbCritical + vbOKOnly
               Exit Function
            End If
        End With
...
Private Function fnInsertLine(vData() As Variant) As Boolean
    ...
    strSQL = "INSERT INTO zzse_gf_store_data(gfsd_flag, gfsd_series,gfsd_period,gfsd_nbr) VALUES(" _
    ...
    If Not fnExecuteSQL(strSQL, SUB_NAME, "") Then
        MsgBox "Failed to insert records into the zzse_gf_store_data table", vbCritical + vbOKOnly
        Exit Function
    End If
...
Private Function fnUpdateLine(vData() As Variant) As Boolean
    ...
    strSQL = "UPDATE zzse_gf_store_data SET  gfsd_nbr=" & tfnSQLString(vData(COL_NBR)) _
             & " WHERE gfsd_flag=" & tfnSQLString(t_nCurrentRpt) _
             & " and gfsd_series = " & tfnSQLString(txtSeries.Text) _
             & " and gfsd_period = " & tfnSQLString(vData(COL_PERIOD))
 
    If Not fnExecuteSQL(strSQL, SUB_NAME, "") Then
        MsgBox "Failed to update records to the zzse_gf_store_data table", vbCritical + vbOKOnly
        Exit Function
    End If
...
Private Function fnDeleteAPeriod(ByVal sPeriod As String) As Boolean
    ...
    strSQL = "DELETE FROM zzse_gf_store_data WHERE gfsd_flag = " & tfnSQLString(t_nCurrentRpt) _
            & " and gfsd_series = " & tfnSQLString(txtSeries.Text) _
            & " and gfsd_period = " & tfnSQLString(sPeriod)
   
    If Not fnExecuteSQL(strSQL, SUB_NAME, "") Then
        MsgBox "Failed to delete records from the zzse_gf_store_data table", vbCritical + vbOKOnly
        Exit Function
    End If
