...
'#569178
Public Const TRACK_COLLECTION_ACTIVITY_UP = 15500  'CLETCCAM.EXE (Track Collection Activity)
...
Public Const NOPRINTERSFOUND As String = "No Printers"
...
'david 12/17/2009  SYWINWEB added Optional bCheckDBVersion As Boolean = True
Public Function tfnOpenDatabase(Optional bShowMsgBox As Boolean = True, _
    ...
    ' CR30842 - Papco - LOCK MODE TO WAIT
    subSetLockModeToWait
...
#If Not NO_DST Then
    ...
    If vColumArr(0, 0) = "" Then
        ...
        If fnCheck_txh_origin_st Then
            vColumArr(16, 0) = "tt_origin_st"
            vColumArr(16, 1) = "txh_origin_st"
        End If
    End If
...
'Vijaya on 11/11/2009 #647512
Public Function fnCheck_txh_origin_st()

    Static bAlreadyCheck            As Boolean
    Static bExists_txh_origin_st    As Boolean
    
    Dim strSQL As String
    Dim rsTemp As Recordset
    
    If Not bAlreadyCheck Then
            strSQL = "SELECT colname FROM systables, syscolumns"
            strSQL = strSQL + " WHERE systables.tabid = syscolumns.tabid"
            strSQL = strSQL + " AND tabname = " + tfnSQLString("tx_header")
            strSQL = strSQL + " AND colname = " + tfnSQLString("txh_origin_st")
...
'*************************************************************************
'Gemini #6694 Daryl Bishop 10/9/2013
'Get customer printers from hostmstr (factor global) database to limit
'printer select list to only those for customer.
'!!!!Expects to be in hosted environment!!!!
'*************************************************************************
Public Function fnGetMasterPrinterRecords() As String()
    Dim sPrinters() As String
    
    ReDim sPrinters(0)
    
    If Len(printerList) < 1 Then
        printerList = CallForPrinterList()
    End If
    
    If printerList <> "" And printerList <> NOPRINTERSFOUND Then
...
'*************************************************************************
'Gemini #6694 Daryl Bishop 10/9/2013
'Get customer printers from hostmstr (factor global) database, if any
'Compare submitted printer to only those listed for customer.
'!!!!Expects to be in hosted environment!!!!
'*************************************************************************
Public Function fnIsMasterPrinterRecords(sPrinterName As String) As Boolean
    On Error GoTo SQLError
    
    Dim sMstrPrinterList() As String
    Dim i As Integer
    Dim sTemp As String
    sMstrPrinterList = fnGetMasterPrinterRecords()
    'Debug.Print printerList
    ...
    For i = 0 To UBound(sMstrPrinterList)
        If Trim$(LCase$(sMstrPrinterList(i))) = sTemp Then
            fnIsMasterPrinterRecords = True
            Exit Function
        End If
    Next

SQLError:
    Debug.Print "error in fnIsMasterPrinterRecords: " & Err.Description
...
' CR30842 - Papco - LOCK MODE TO WAIT
Private Sub subSetLockModeToWait()
    On Error GoTo FINISHED
    Dim strSQL As String
    
    If tfnConnectionIsIDS() Then
        strSQL = "SET LOCK MODE TO WAIT 45"
        t_dbMainDatabase.ExecuteSQL strSQL
        If Not io Is Nothing Then
            On Error Resume Next
            tfnLog strSQL, io.StandardLogFile
            On Error GoTo FINISHED
        End If
    End If
...
FINISHED:
    If Err.Number <> 0 Then
        MsgBox "Error in subSetLockModeToWait(): " & _
...
' CR30842 - Papco - LOCK MODE TO WAIT
Public Function tfnConnectionIsIDS() As Boolean
    On Error GoTo FINISHED
    Dim sDBPath As String
    
    If t_dbMainDatabase Is Nothing Then Exit Function
    If Trim(t_dbMainDatabase.Connect) = "" Then Exit Function
    
    sDBPath = tfnGetDbName(True)
    If Trim(sDBPath) = "" Then Exit Function
    
    If InStr(1, sDBPath, "/") < 1 Then
        tfnConnectionIsIDS = True
    End If
    
    Err.Clear
FINISHED:
    If Err.Number <> 0 Then
        MsgBox "Error in tfnConnectionIsIDS(): " & _
               Err.Number & vbCrLf & Err.Description
        Err.Clear
    End If
End Function
